...
class CloudFirestore {
    ...
    func fetchChannels(completion: @escaping (Channel?, ErrorsManager?) -> Void) {

        chatRooms.getDocuments { (snapShot, error) in
            
            guard error == nil else {completion(nil, ErrorsManager.failedFetchingChannels); return }
            _ = snapShot?.documents.map({ (document) in
                ...
                documentContent.forEach { (key, _) in
                    guard let channelDescription = documentContent[key] as? String else { return }
