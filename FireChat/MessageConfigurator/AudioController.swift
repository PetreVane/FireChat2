...
public enum AudioPlayerState {
    ...
    case playing
    /// The audio controller is currently paused
    case pause
    /// The audio controller is currently stopped and audioPlayer is nil
    case stopped
}


// The `AudioController` updates the UI for current audio cell that is playing a sound
/// and also creates and manages an `AVAudioPlayer` states, play, pause and stop.
open class AudioController: NSObject, AVAudioPlayerDelegate {
    ...
    public func configureAudioCell(_ cell: AudioMessageCell, message: MessageType) {
        if
            playingMessage?.messageId == message.messageId,
            ...
            guard let displayDelegate = collectionView.messagesDisplayDelegate else { fatalError("MessagesDisplayDelegate has not been set.") }
    ...
    public func playSound(for message: MessageType, in audioCell: AudioMessageCell) {
        ...
        case .audio(let item):
            ...
            guard let player = try? AVAudioPlayer(contentsOf: item.url) else { print("Failed to create audio player for URL: \(item.url)"); return }
            audioPlayer = player
    ...
    public func stopAnyOngoingPlaying() {
        guard let player = audioPlayer, let collectionView = messageCollectionView
    ...
    open func resumeSound() {
        guard let player = audioPlayer, let cell = playingCell else { stopAnyOngoingPlaying(); return }
    ...
    @objc private func didFireProgressTimer(_ timer: Timer) {
        guard let player = audioPlayer, let collectionView = messageCollectionView, let cell = playingCell else { return }
